...
contract CrossMatchingAdapterFeeTest is Test, TestHelper {
    ...
    function test_crossMatchShortOrders_mixedBuySellOrders() public {
        ...
        // mint yes tokens to user2 for question2
        _mintTokensToUser(user2, yesPositionId2, fillAmount);
        vm.prank(user2);
        ctf.setApprovalForAll(address(adapter), true);

        // Create mixed maker orders (2 BUY, 2 SELL)
        ICTFExchange.OrderIntent[] memory makerOrders = new ICTFExchange.OrderIntent[](3);
        makerOrders[0] = _createOrderIntent(
            user2,
            questionId2,
        ...
        // mint yes tokens to user3 for question3
        _mintTokensToUser(user3, yesPositionId3, fillAmount);
        vm.prank(user3);
        ctf.setApprovalForAll(address(adapter), true);
        ...
        // mint yes tokens to user4 for question4
        _mintTokensToUser(user4, yesPositionId4, fillAmount);
        vm.prank(user4);
        ctf.setApprovalForAll(address(adapter), true);
