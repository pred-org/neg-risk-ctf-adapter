...
contract CrossMatchingAdapterHybridSingleOrdersTest is Test, TestHelper {
    ...
    function testSingleOrdersMergeOnePriceMismatchTest() public {
      ...
      // For sell order: makerAmount = token amount (1e6), takerAmount = USDC amount (0.55e6)
      // price = 0.45$
      OrderIntent memory takerOrder = _createAndSignOrder(user2, noPositionId, 1, 1e6, 0.5e6, questionId, 0, _user2PK);

      vm.expectRevert(abi.encodeWithSelector(ITradingEE.NotCrossing.selector));
      // Execute hybrid match orders (1 single order)
      adapter.hybridMatchOrders(marketId, takerOrder, makerOrders, takerFillAmounts, 1);

      console.log("Single orders merge one price mismatch test passed!");
    }
    ...
    function testSingleOrdersMintOneInsufficientBalanceTest() public {
      console.log("=== Testing Single Orders Mint One Insufficient Balance Test ===");
    ...
    function testSingleOrdersMergeMultipleTest() public {
      ...
      adapter.hybridMatchOrders(marketId, takerOrder, makerOrders, takerFillAmounts, 3);
      ...
      for (uint256 i = 0; i < 3; i++) {
        ...
        // For SELL order: loses makerFillAmount tokens = 1e6 YES tokens
        assertEq(ctf.balanceOf(maker, yesPositionId), initialMakerYES[i] - makerOrders[i].makerFillAmounts[0], 
    ...
    function testSingleOrdersComplementaryMultipleTest() public {
      ...
      for (uint256 i = 0; i < 3; i++) {
        ...
        assertEq(ctf.balanceOf(maker, noPositionId), makerOrders[i].makerFillAmounts[0] * makerOrders[i].orders[0].takerAmount / makerOrders[i].orders[0].makerAmount, 
      ...
      for (uint256 i = 0; i < 3; i++) {
        ...
        assertEq(usdc.balanceOf(maker), initialMakerUSDC[i] - makerOrders[i].makerFillAmounts[0], 
    ...
    function testSingleOrdersMintMultipleTest() public {
      ...
      for (uint256 i = 0; i < 3; i++) {
        ...
        assertEq(ctf.balanceOf(maker, yesPositionId), makerOrders[i].makerFillAmounts[0] * makerOrders[i].orders[0].takerAmount / makerOrders[i].orders[0].makerAmount, 
